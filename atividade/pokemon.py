...
def nome_do_pokemon(numero):
    ...
    # dicionário = chamaABiblioteca.funçãoGetParaTrazerInfos(URL).transformaTxtEmJson()
    return dic["name"]
...
def numero_do_pokemon(nome):
    r = requests.get(f"http://pokeapi.co/api/v2/pokemon/{nome.lower()}/")
...
def color_of_pokemon(nome):
    r = requests.get(f"https://pokeapi.co/api/v2/pokemon-species/{nome.lower()}/")
    if r.status_code != 200:
        raise PokemonNaoExisteException
    dic = r.json()
...
def cor_do_pokemon(nome):
    ...
    if r.status_code != 200:
        raise PokemonNaoExisteException
    dic = r.json()
    return dic_cores[dic["color"]["name"]]
...
def tipos_do_pokemon(nome):
    r = requests.get(f"https://pokeapi.co/api/v2/pokemon/{nome.lower()}/")
    if r.status_code != 200:
        raise PokemonNaoExisteException
    dic = r.json()
    types = []
    for i in dic["types"]:
        types.append(dic_tipos[i["type"]["name"]])
    return types
...
def nivel_do_pokemon(nome, experiencia):
    ...
    if r.status_code != 200:
        raise PokemonNaoExisteException
    dic_species = r.json()
    dic_growth_rate = requests.get(dic_species["growth_rate"]["url"]).json()
    nivel_anterior = {}
    for i in dic_growth_rate["levels"]:
        if experiencia > dic_growth_rate["levels"][-1]["experience"]:
            return dic_growth_rate["levels"][-1]["level"]
        if i["experience"] == experiencia:
            return i["level"]
        if i["experience"] > experiencia:
            return nivel_anterior["level"]
        nivel_anterior = i
...
def ganhar_experiencia(nome_treinador, apelido_pokemon, experiencia):
    r = requests.post(f"{site_treinador}/treinador/{nome_treinador}/{apelido_pokemon}/exp", json = {"experiencia": experiencia})
...
* nome treinador (veio como argumento da funcao) ok
* apelido pokemon (veio como argumento da funcao) ok
* tipo (veio do get que você fez -- chave tipo do dicionário) ok
* experiencia (veio do request que você fez -- chave experiencia do dicionário) ok
* nivel do pokemon (calcular usando a pokeapi -- voce ja fez essa funcao, use ela) ok
...
def detalhar_treinador(nome_treinador):
    r = requests.get(f"{site_treinador}/treinador/{nome_treinador}")
    if r.status_code == 404:
        raise TreinadorNaoCadastradoException
    dic = r.json()["pokemons"]
    pokemons = {}
    for i in dic:
        pokemons[dic[i]["apelido"]] = dic[i]["tipo"]
    return pokemons
...
def excluir_treinador(nome_treinador):
    ...
    if r.status_code == 404:
        raise TreinadorNaoCadastradoException
    r = requests.delete(f"{site_treinador}/treinador/{nome_treinador}")
...
def excluir_pokemon(nome_treinador, apelido_pokemon):
    r = requests.get(f"{site_treinador}/treinador/{nome_treinador}/{apelido_pokemon}")
    if r.status_code != 404:
        r = requests.delete(f"{site_treinador}/treinador/{nome_treinador}/{apelido_pokemon}")
    else:
        erro = requests.get(f"{site_treinador}/treinador/{nome_treinador}")
        if erro.status_code == 404:
            raise TreinadorNaoCadastradoException
